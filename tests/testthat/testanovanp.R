...
testthat::test_that('All options in the anovaNP work (sunny)', {
    ...
    r <- jmv::anovaNP(data, deps='dep', group='f', es=TRUE, postHocMethod="dscf")
    ...
    r <- jmv::anovaNP(data, deps='dep', group='f', es=TRUE, postHocMethod="dunn")

    # Test comparisons table (Dunn)
    comp <- r$dunnComparisons[[1]]$asDF
